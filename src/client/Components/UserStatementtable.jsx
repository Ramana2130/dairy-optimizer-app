...
import { ChevronLeft, ChevronRight, ArrowDownToLine, ScrollText } from "lucide-react";
import '../index.css';
...
const UserStatementtable = () => {
    ...
    if (customerId) {
      const fetchData = async () => {
        try {
          ...
          const milkData = response.data.milkDetails || []; // Correctly access milkDetails
          const adminData = response.data.admin || null; // Correctly access admin

          // Create a map from admin ID to username
          const adminMap = {};
          if (adminData) {
  ...
  const totalPages = Math.ceil(Array.isArray(data) ? data.length / rowsPerPage : 0);
                ...
                <table className="table-auto">
                  <thead className=" border-b ">
                    <tr className="bg-transparent">
                      ...
                      <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase">S.No</th>
                      <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase min-w-[150px]">Consumer Name</th>
                      <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase">Delivery Schedule</th>
                      <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase">Quantity</th>
                      <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase">Price</th>
                      <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase">Date & Time</th>
                      <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase">Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white">
                    {currentRows.length > 0 ? (
                      currentRows.map((row, index) => (
                        <tr key={row._id} className="transition-all duration-500 hover:bg-black/10">
                          <td className="p-5">
                      ...
                      <tr>
                        <td colSpan="8" className="p-5 text-center text-black">No data available</td>
                      </tr>
                    )}
                  </tbody>
                </table>
