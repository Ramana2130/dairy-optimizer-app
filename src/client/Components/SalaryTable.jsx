              ...
              <div className="overflow-hidden flex  space-x-16  h-[75vh] w-[75vw]  p-10 rounded-xl  shadow-2xl backdrop-blur">
                <div className="w-[50vw] ">
                  <div className="p-5 mx-auto flex justify-center leading-6 font-medium text-gray-900">
                    ...
                    >
                      <span className="font-medium text-center text-2xl">User Income List</span>
                    </div>
                  </div>
                  <div className="p-5">
                  ...
                  <table className="table-auto w-full">
                    <thead>
                      <tr className="bg-transparent">
                        ...
                        <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase min-w-[150px]">User Name</th>
                        <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase">Period</th>
                        <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase">Total Quantity</th>
                        <th className="p-5 text-left text-sm leading-6 font-semibold text-black uppercase">Total Price</th>
                      </tr>
                    </thead>
                    <tbody>
                      {currentRows.map((username, index) => {
                        const periodTotals = getPeriodTotals(username);
                        return Object.keys(periodTotals).map((period, periodIndex) => (
                          <tr key={`${username}-${period}`} className="transition-all duration-500 hover:bg-black/5 border-b  border-black/20">
